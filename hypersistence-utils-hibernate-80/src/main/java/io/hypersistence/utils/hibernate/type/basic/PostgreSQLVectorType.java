...
public class PostgreSQLVectorType extends ImmutableType<float[]> {
    ...
    @Override
    public float[] get(ResultSet rs, int position, WrapperOptions options) throws SQLException {
        Object value = rs.getObject(position);
        return value == null ? null : fromStringValue(value.toString());
    }

    @Override
    public void set(PreparedStatement st, float[] value, int index, WrapperOptions options) throws SQLException {
        if (value == null) {
